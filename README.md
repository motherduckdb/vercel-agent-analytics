...
Deploy a collector for Vercel Log Drains that classifies AI crawlers, AI agents, and visitors arriving from AI products, then writes the request stream to MotherDuck.
...
- Accepts log drains from multiple Vercel projects.
...
- Stores Vercel project metadata as `project_id`, `project_name`, and `deployment_id`.
...
| `VERCEL_DRAIN_SECRET` | Shared HMAC secret. Use the same value when creating your first Vercel Log Drain. |

Optional variables:

| Variable | Default | Description |
| --- | --- | --- |
| `VERCEL_DRAIN_SECRETS` | unset | Comma- or newline-separated list of accepted drain secrets. Use this when multiple monitored Vercel projects should each have their own secret. |
...
Deploying this template only creates the collector. It does not automatically monitor any other Vercel project. For each project you want to analyze, add a Log Drain that points to the collector.

In the Vercel dashboard for each app you want to monitor:
...
For multiple projects, you can either reuse one shared secret or give each project its own secret. To use per-project secrets, add every accepted secret to `VERCEL_DRAIN_SECRETS`:

```text
docs-drain-secret,app-drain-secret,marketing-drain-secret
```

`VERCEL_DRAIN_SECRET` and `VERCEL_DRAIN_SECRETS` are both accepted. If both are set, the collector accepts signatures generated with any listed secret.
...
Project fields are stored on every row when Vercel includes them:
